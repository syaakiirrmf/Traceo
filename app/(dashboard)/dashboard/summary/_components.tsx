...
export function StatusBadge({ status }: { status: keyof typeof STATUS_CONFIG | string }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium border ${config.bg} ${config.text} ${config.border}`}
    >
      {config.label}
    </span>
  )
  ...
  return (
    ...
    >
      ...
      <span>
        {showExtra
          ? 'Hide extra details'
          : 'Show extra details (Nominee & Transfer of Title Status)'}
      </span>
    </button>
  )
}
