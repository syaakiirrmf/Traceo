...
export function TanahMDTable({ rows }: { rows: Partial<TanahJV>[] }) {
  ...
  const districts = useMemo(
    () =>
      [...new Set(rows.filter((t) => !stateFilter || t.negeri === stateFilter).map((t) => t.daerah).filter(Boolean))]
        .sort() as string[],
    [rows, stateFilter]
  )
  ...
  const visibleRows = useMemo(() => {
    return rows.filter((t) => {
      ...
      if (!q) return true
      return [t.no_lot, t.negeri, t.daerah, t.bandar_mukim, t.tempat, t.no_hak_milik, t.catatan].some((v) =>
        matchesQuery(v, q)
      )
    })
        ...
        <table className="w-full text-xs text-left border-collapse">
          <thead className="sticky top-0 z-30 bg-[var(--color-surface-raised)] border-b border-[var(--color-border)] shadow-xs">
            ...
            <tr className="border-b border-[var(--color-border)] text-[11px] text-[var(--color-text-tertiary)] uppercase tracking-wider bg-[var(--color-surface-raised)]">
              <th className="px-3.5 py-2 font-medium border-r border-[var(--color-border)]">State</th>
              <th className="px-3.5 py-2 font-medium border-r border-[var(--color-border)]">District</th>
              <th className="px-3.5 py-2 font-medium border-r border-[var(--color-border)]">Town / Village / Mukim</th>
              <th className="px-3.5 py-2 font-medium border-r border-[var(--color-border)]">Place</th>
              <th className="px-3.5 py-2 font-medium border-r border-[var(--color-border)]">Registered On</th>
              <th className="px-3.5 py-2 font-medium border-r border-[var(--color-border)]">Title Number</th>
            </tr>
          </thead>
